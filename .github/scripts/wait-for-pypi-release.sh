...
# `uv pip install --no-cache-dir "th2etl[postgres]==$VERSION"`. PyPI's
# index takes some time to propagate a freshly published release, so
# building right away is a race, failing with:
#
#   Because there is no version of th2etl==X and you require
#   th2etl==X, we can conclude that your requirements are unsatisfiable.
#
# This guard is ported verbatim from apowerb/th2pulse, itself from
# apowerb/apowerb, where releases 0.1.11 and 0.1.12 both hit that race and
# both succeeded on manual re-run once PyPI had caught up. Cutting th2pulse
# 0.1.2 by hand hit it too.
#
...
#                      installs them, e.g. th2etl[postgres]
...
for attempt in $(seq 1 "$MAX_ATTEMPTS"); do
  ...
  # `set -u` is an unbound-variable error in bash 3.2, which is what macOS
  # ships, and this script's header invites local runs. CI is bash 5.2 and
  # never saw it.
  if uv pip install --system --break-system-packages --dry-run \
      ${extra_uv_args[@]+"${extra_uv_args[@]}"} "$requirement"; then
